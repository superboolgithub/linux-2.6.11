...
void driver_attach(struct device_driver * drv)
{
	...
	list_for_each(entry, &bus->devices.list) {//遍历该总线上的所有设备
		struct device * dev = container_of(entry, struct device, bus_list);
		if (!dev->driver) {//该设备没有驱动，还未被内核支持
			error = driver_probe_device(drv, dev);//用bus_type->match()看是否匹配
...
int bus_add_driver(struct device_driver * drv)
{
	...
	if (bus) {
		...
		if ((error = kobject_register(&drv->kobj))) { //把驱动插入设备驱动程序模型，/sys目录下
			put_bus(bus);
			return error;
		}
