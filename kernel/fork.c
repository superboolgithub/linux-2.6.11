	      ...
	      int __user *child_tidptr)
{
	...
	/* 复制父进程的task_struct，并申请了内核栈和thread_info */
	...
	if (!IS_ERR(p)) {
		...
		/* 如果设置了CLONE_STOPPED标志，或要跟踪子进程，那么子进程被设置成TASK_STOPPED,并为子进程增加挂起的SIGSTOP信号，在灵异进程把子进程的状态恢复为TASK_RUNNING之前（通常是SIGCONT信号），子进程不得运行*/
